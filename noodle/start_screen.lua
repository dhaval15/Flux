...
local user_picture = wibox.widget {
    {
        image  = os.getenv("HOME").."/.config/awesome/profile.jpg",
        resize = false,
        clip_shape = gears.shape.circle,
        widget = wibox.widget.imagebox
    },
    -- wibox.widget.imagebox(os.getenv("HOME").."/.config/awesome/profile.jpg"),
    widget = user_picture_container
}
local username = os.getenv("USER")
local user_text = wibox.widget.textbox()
user_text.markup = helpers.colorize_text(username, beautiful.xcolor8)
-- Capitalize username
...
local function create_bookmark(name, path)
    ...
    bookmark:buttons(
      gears.table.join(
        awful.button({ }, 1, function ()
            awful.spawn.with_shell("nautilus "..path)
            start_screen_hide()
        end),
